...
function PowerSats.LaunchPowerSat(_force, _surface, _sat_type)
  ...
  if global.powersats["satellite_data"][_force.index] == nil then global.powersats["satellite_data"][_force.index] = {} end
  local force_table = global.powersats["satellite_data"][_force.index]
  ...
  if surface_table["power_multiplier"] == nil then
    if PowerSats.SE then
      ...
      surface_table["power_multiplier"] = global.powersats["data"]["solarSatMultiplier"]
    end
  end
...
function PowerSats.UpdateTick()
  for k, v in pairs(global.powersats["satellite_data"]) do
...
function PowerSats.GetSurfacePowerMultiplier(_force_index, _surface_index)
  return global.powersats["satellite_data"][_force_index][_surface_index]["power_multiplier"]
end
...
function PowerSats.GroundStationBuilt(_entity)
  ...
  if global.powersats["ground_stations"]["station_data"] == nil then global.powersats["ground_stations"]["station_data"] = {} end
  local stationTable = global.powersats["ground_stations"]["station_data"]
  ...
  local totalCount = global.powersats["ground_stations"]["count"] or 0
  totalCount = totalCount + 1
  global.powersats["ground_stations"]["count"] = totalCount
...
function PowerSats.GroundStationRemoved(_entity)
  ...
  local surfaceTable = global.powersats["ground_stations"]["station_data"][_entity.force.index][_entity.surface.index]
  PowerSats.RemoveStation(surfaceTable, _entity)
  surfaceTable["count"] = surfaceTable["count"] - 1
  
  global.powersats["ground_stations"]["count"] = global.powersats["ground_stations"]["count"] - 1
end
...
function PowerSats.Tick(_tick)
  ...
  for k, v in pairs(global.powersats["ground_stations"]["station_data"]) do
...
function PowerSats.GetPowerGeneration(_force_index, _surface_index)
  ...
  if global.powersats["satellite_data"][_force_index] == nil then return 0 end
  
  if global.powersats["satellite_data"][_force_index][_surface_index] == nil then return 0 end

  return global.powersats["satellite_data"][_force_index][_surface_index]["power_generated"]
...
function PowerSats.CombinatorBuilt(_entity)
  ...
  if global.powersats["combinators"][_entity.force.index] == nil then global.powersats["combinators"][_entity.force.index] = {} end
  local forceTable = global.powersats["combinators"][_entity.force.index]
...
function PowerSats.CombinatorRemoved(_entity)
  ...
  local surfaceTable = global.powersats["combinators"][_entity.force.index][_entity.surface.index]
...
function PowerSats.UpdateCombinators(_force, _surface)
  ...
  if (global.powersats["combinators"][_force]) == nil then return end
  
  if (global.powersats["combinators"][_force][_surface]) == nil then return end

  for k,v in pairs(global.powersats["combinators"][_force][_surface]) do
    if global.powersats["satellite_data"][_force] ~= nil and global.powersats["satellite_data"][_force][_surface] ~= nil then
  
      for l,b in pairs(global.powersats["satellite_data"][_force][_surface]["satellite_data"]) do
        local count = b["count"]        
        local combinator = v.get_or_create_control_behavior()        
        combinator.set_signal(1, { signal = { type = "item", name = "powerSat" }, count = count })
      end
